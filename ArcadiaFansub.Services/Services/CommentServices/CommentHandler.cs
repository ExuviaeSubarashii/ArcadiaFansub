...
namespace ArcadiaFansub.Services.Services.CommentServices
{
	public class CommentHandler(ArcadiaFansubContext AF, UserAuthentication auth) : ICommentInterface
	{
		...
		public async Task<string> DeleteAllComments(string episodeId, CancellationToken cancellationToken)
		{
			var animeQuery = await AF.Comments.Where(x => x.EpisodeId.Contains(episodeId.Trim())).ToListAsync(cancellationToken);
		...
		public async Task<string> DeleteEpisodeComment(DeleteEpisodeCommentBody body, CancellationToken cancellationToken)
		{
			...
			if (commentQuery != null && IsCommentOwner.IsOwner(body.UserToken, userQuery.UserId) || await auth.IsAdmin(body.UserToken.Trim()) && commentQuery != null)
			{
		...
		public async Task<string> UpdateEpisodeComment(UpdateEpisodeCommentBody body, CancellationToken cancellationToken)
		{
			...
			{
				if (IsCommentOwner.IsOwner(body.UserToken, commentQuery.UserId))
				{

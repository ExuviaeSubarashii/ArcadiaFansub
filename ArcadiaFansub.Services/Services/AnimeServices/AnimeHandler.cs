	...
	public class AnimeHandler(ArcadiaFansubContext AF, CommentHandler CH, EpisodeHandler EH) : IAnimeInterface
	{
		...
		public async Task<string> AddOrRemoveAnimeToFavorites(string animeId, string userToken, CancellationToken cancellationToken)
		{
			...
			if (userQuery != null)
			{
				...
				List<string> favoritedList = userQuery.FavoritedAnimes != null ? userQuery.FavoritedAnimes.Split(',').ToList() : new List<string>();
				favoritedList.RemoveAll(x => x == "");
		...
		public async Task<string> UpdateAnimeProperties(UpdateAnimeRequest updateAnimeRequest, CancellationToken cancellationToken)
		{
			try
			{
				...
				animeQuery.AnimeEpisodeAmount = (int)(updateAnimeRequest.NewEpisodeAmount != 0 || (int)updateAnimeRequest.NewEpisodeAmount != null ? updateAnimeRequest.NewEpisodeAmount : animeQuery.AnimeEpisodeAmount);

				animeQuery.AnimeName = !string.IsNullOrEmpty(updateAnimeRequest.NewAnimeName) ? updateAnimeRequest.NewAnimeName : animeQuery.AnimeName;

				animeQuery.Editor = !string.IsNullOrEmpty(updateAnimeRequest.NewEditorName) ? updateAnimeRequest.NewEditorName : animeQuery.Editor;

				animeQuery.Translator = !string.IsNullOrEmpty(updateAnimeRequest.NewTranslatorName) ? updateAnimeRequest.NewTranslatorName : animeQuery.Translator;
		...
		public async Task<string> DeleteAnimeFromEverybody(string animeId, CancellationToken cancellationToken)
		{
			...
			var usersQuery = await AF.Users.Where(x => x.FavoritedAnimes != null && x.FavoritedAnimes.Contains(animeId)).ToListAsync();
			foreach (var user in usersQuery)
			{
				if (user.FavoritedAnimes != null)
				{

...
namespace ArcadiaFansub.API.Controllers
{
	...
	public class UserController(UserHandler userHandler, UserAuthentication authHandler) : ControllerBase
	{
		...
		[HttpPost("GetUserBySearch/{param}")]
		public async Task<IActionResult> GetUserBySearch(string param, CancellationToken cancellationToken)
		{
			return await (userHandler.SearchUser(param, cancellationToken)) is { } result ? Ok(result) : NotFound();
		}
	}
}
